            ...
            return DataTables::of($roles)
                ...
                ->addColumn('actions', function ($roles) {
                    ...
                    if (Auth::user()->can('grant-permit')) {
                        $url = route('role.show-permit', $roles['id']);

                        $actions .= "<a href='javascript:void(0)' class='btn btn-sm btn-warning mb-1 mr-1 grant-permissions' title='Granted permissions'  data-url='" . $url . "' data-toggle='modal' data-target='#modalFormPermitGranting'><i class='las la-lg la-key'></i></a>";
                    }
                    if (Auth::user()->can('edit-role')) {
                        $url = route('role.edit', $roles['id']);

                        $actions .= "<a href='javascript:void(0)' class='btn btn-sm btn-secondary mb-1 mr-1 edit' title='Edit data' data-url='" . $url . "' data-toggle='modal' data-target='#modalFormRole'>Edit</a>";
                    }
